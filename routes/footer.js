...
router.post('/newsletter', function (req, res) {
    ...
    const request = https.request(url, options, function (response) {
        if (response.statusCode === 200) {
            req.flash(
                'success',
                "You've been succeessfully subscribed to the Cyclone Analytics newsletter!"
            );
            return res.redirect('/home');
        } else {
            req.flash(
                'error',
                "There's been an issue with your signup, maybe you're already subscribed!"
            );
            return res.redirect('/home');
        }
        ...
        Blog.find({ name: regex }, function (err, allBlogs) {
            if (err) {
                ...
                if (allBlogs.length < 1) {
                    return res.render('footer/blog', {
                        blogs: allBlogs,
                        error: 'No match! Please try again!',
                    });
                }
                res.render('footer/blog', {
                    blogs: allBlogs,
        ...
        .exec(function (err, foundBlog) {
            if (err) {
                ...
                Blog.find({})
                    .sort('-createdAt')
                    .limit(3)
                    .exec(function (err, recentBlog) {
                        if (err) {
                            console.log(err);
                        } else {
                            // Render show template with requested blog
                            res.render('footer/show', {
                                blog: foundBlog,
                                recent: recentBlog,
                                currentUser: req.user,
                            });
